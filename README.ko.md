...
| **명확화** | 잘못된 것을 만들지 않기 위해 꼭 필요한 질문만 수행 |
...
| **실행** | TDD 원칙으로 구현 — 실패하는 테스트 먼저, 최소한의 수정, 그린 확인. GREEN은 완료가 아닌 리뷰로의 mandatory transition을 트리거 |
| **리뷰** | diff 기반의 강한 리뷰 루프를 actionable findings가 0이 될 때까지 반복. 패치할 때마다 변경된 diff에 대해 fresh re-review 필수 — 패치 자체가 리뷰를 대체하지 않음 |
| **검증** | 새로운 증거 기반 검증 수행 — "아마 될 거예요"는 허용되지 않음 |
...
- **최소한의 질문** — 잘못된 것을 만들 수 있는 경우에만 질문
...
- **`references/`** — 단계별 가이드. 각 파일은 워크플로우의 한 단계를 실행하는 방법을 상세히 다룹니다.
