...
| **실행** | TDD 원칙으로 구현 — 실패하는 테스트 먼저, 최소한의 수정, 그린 확인. Pre-edit investigation gate로 편집 전 파일 읽기 강제. 테스트 실패 시 in-branch/pre-existing/flaky로 분류 후 조사. non-micro 구현 작업은 discovery 후 subagent 도구가 없거나 안전한 bounded slice가 없는 경우를 제외하고 최소 1개의 fresh subagent를 dispatch. GREEN은 완료가 아닌 리뷰로의 mandatory transition을 트리거 |
...
| **검증** | 4단계 검증 (Exists → Substantive → Wired → Functional) + stub 감지 패턴. 필요 시 user-flow, onboarding/DX, migration/codegen, abuse-path, E2E 검증까지 포함. persistence를 건드리는 E2E는 기본적으로 격리된 SQLite test DB를 사용. "아마 될 거예요"는 허용되지 않음 |
...
- **non-micro 작업의 subagent 강제** — non-micro 구현 작업은 도구가 없거나 안전한 bounded slice가 없는 경우를 제외하고 최소 1개의 fresh subagent를 dispatch; 구현 위임이 위험하면 조사, 테스트 설계, 1차 리뷰 위임으로 대체
...
- **필요할 때 E2E** — 통합된 사용자-facing 동작은 E2E를 proving check 후보로 평가; persistence를 건드리는 E2E는 기본적으로 disposable SQLite를 사용하고, 명시 승인 없이는 production/shared staging/일반 개발 DB를 사용하지 않음
...
- requirement coverage 보존, worktree 선택, non-micro 구현 작업의 fresh subagent dispatch도 `superpilot`이 맡긴다.
- 통합된 사용자-facing 동작은 E2E를 proving check 후보로 평가하고, persistence를 건드리는 E2E는 기본적으로 격리된 SQLite test DB를 사용한다.
