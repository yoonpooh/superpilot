...
| **실행** | TDD 원칙으로 구현 — 실패하는 테스트 먼저, 최소한의 수정, 그린 확인 |
| **리뷰** | diff 기반의 강한 리뷰 루프를 actionable findings가 0이 될 때까지 반복하고, 구현 작업이면 완료 전에 루프 안에서 findings를 직접 패치 |
...
- **리뷰 우선 완료** — 테스트가 다시 통과해도 fresh final review pass를 대신할 수 없음
