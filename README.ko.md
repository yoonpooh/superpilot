...
| 단계 | 설명 |
|---|---|
| **탐색** | 저장소 맥락, 로컬 규칙(`AGENTS.md`, `CLAUDE.md`), 최근 변경사항 파악 |
| **명확화** | 코드 기반 가정을 먼저 제시하고, 사용자가 틀린 부분만 교정하는 방식으로 질문 최소화 |
| **설계** | 접근법을 trade-off와 함께 제안하고, 하나를 추천하며, 사용자와 협업 |
| **Spec** | 목표, 범위, 설계, 엣지 케이스, 테스트 전략을 담은 구체적인 spec 작성 |
| **Plan** | 파일 대상, 테스트 기대치, 검증 단계를 포함한 실행 가능한 작업으로 분해 |
| **디버깅** | 버그 관련 작업인 경우: 수정 제안 전에 증거 기반으로 근본 원인 조사 |
| **실행** | TDD 원칙으로 구현 — 실패하는 테스트 먼저, 최소한의 수정, 그린 확인. Pre-edit investigation gate로 편집 전 파일 읽기 강제. 테스트 실패 시 in-branch/pre-existing/flaky로 분류 후 조사. GREEN은 완료가 아닌 리뷰로의 mandatory transition을 트리거 |
| **리뷰** | diff 기반의 강한 리뷰 루프 + stall detection (3회 연속 findings 미감소 시 에스컬레이션). 대규모 diff에는 병렬 전문가 리뷰 (Security, Performance, API Contract, Data Consistency, Test Coverage) 옵션. 패치마다 fresh re-review 필수 |
| **검증** | 4단계 검증 (Exists → Substantive → Wired → Functional) + stub 감지 패턴. "아마 될 거예요"는 허용되지 않음 |

Spec을 올바르게 작성할 수 있을 만큼 충분히 명확해지면, 원래 사용자 요청 자체를 실행 승인으로 간주합니다. 에이전트가 중간에 멈추는 경우는 실제 safety gate뿐입니다.
