...
- **non-micro 작업의 subagent 강제** — 모든 non-micro Superpilot 작업은 도구가 없거나 안전한 bounded slice가 없는 경우를 제외하고 최소 1개의 fresh subagent를 dispatch; 사용자가 parallel agent를 별도로 요청할 필요는 없으며, 코드 수정 위임이 위험하면 조사, 테스트 설계, 1차 리뷰 위임으로 대체
...
- requirement coverage 보존, worktree 선택, 모든 non-micro Superpilot 작업의 fresh subagent dispatch도 `superpilot`이 맡긴다.
