...
| **실행** | TDD 원칙으로 구현 — 실패하는 테스트 먼저, 최소한의 수정, 그린 확인. Pre-edit investigation gate로 편집 전 파일 읽기 강제. 테스트 실패 시 in-branch/pre-existing/flaky로 분류 후 조사. 모든 non-micro Superpilot 작업은 discovery 후 subagent 도구가 없거나 안전한 bounded slice가 없는 경우를 제외하고 최소 1개의 fresh subagent를 dispatch. GREEN은 완료가 아닌 리뷰로의 mandatory transition을 트리거 |
...
| **검증** | 4단계 검증 (Exists → Substantive → Wired → Functional) + stub 감지 패턴. 필요 시 user-flow, onboarding/DX, migration/codegen, abuse-path, E2E 검증까지 포함. E2E는 기본적으로 로컬 앱/서버에서 실행하고, persistence를 건드리면 격리된 SQLite test DB를 사용. "아마 될 거예요"는 허용되지 않음 |

명시적 구현 요청에서는 must-have 요구사항이 사용자 확인 또는 코드 증거로 확정되고 Spec을 올바르게 작성할 수 있을 만큼 충분히 명확해지면, 원래 사용자 요청 자체를 실행 승인으로 간주합니다. 에이전트가 중간에 멈추는 경우는 실제 safety gate뿐입니다.
...
- **Micro Task Mode** — 저위험이고 범위가 좁은 작업은 Spec/Plan/TDD는 생략하되, 최소 검증 전 scoped review loop는 실행
- **요구사항 확인 우선** — 전체 워크플로우 spec은 must-have 요구사항이 사용자 확인 또는 코드 증거로 확정된 뒤에만 시작
...
- **non-micro 작업의 subagent 강제** — 모든 non-micro Superpilot 작업은 도구가 없거나 안전한 bounded slice가 없는 경우 fresh subagent를 dispatch; 소유 경계가 분리되면 code-edit 또는 test-edit slice를 우선하고, 테스트/리뷰/검증 slice가 병렬 가능하면 read-only explorer 1개에서 멈추지 않음
- **모든 단계 parallel-first** — 탐색, 조사, 구현, 테스트, 리뷰, 검증에서 독립 slice가 있으면 안전한 범위 안에서 병렬로 실행해 경과 시간을 줄임
- **준수 미충족은 blocker** — 필수 subagent, 리뷰, 검증, 인터랙티브 브라우저 smoke 기준 미충족은 최종 보고 참고사항이 아니라 완료 차단 조건
...
- **필요할 때 로컬 E2E** — 통합된 사용자-facing 동작은 E2E를 proving check 후보로 평가; E2E는 기본적으로 로컬 앱/서버에서 실행하고, persistence는 기본적으로 disposable SQLite를 사용하며, 명시 승인 없이는 배포하거나 production/shared staging/public URL/일반 개발 DB를 사용하지 않음
- **인터랙티브 UI 브라우저 smoke** — 정렬, 필터, 페이지네이션, 탭, 드롭다운, 검색, XHR, 무한스크롤 변경은 focused unit/service test만으로 완료하지 않고 로컬 브라우저 smoke 또는 E2E 증거를 요구
- **격리 브라우저 타깃** — 운영/공유 DB 위험이 있으면 약한 대체 테스트로 완료하지 않고 격리된 로컬 브라우저/E2E 타깃을 먼저 구성
...
- **`SKILL.md`** — 메인 계약서. Superpilot 사용 시점, Micro Task Mode, 전체 워크플로우, 하드 룰, 저장 경로, safety gate를 정의합니다.
...
- 저위험이고 범위가 좁은 작업은 전체 워크플로우 대신 Micro Task Mode를 사용하되, 완료 전 scoped review loop는 실행한다.
- 전체 워크플로우 spec 작성 전 must-have 요구사항을 모두 사용자 확인 또는 저장소 증거로 확정한다.
- research, spec, plan, TDD, 리뷰, 검증 흐름은 `superpilot`을 단일 기준으로 삼는다.
- requirement coverage 보존, worktree 선택, 모든 non-micro Superpilot 작업의 fresh subagent dispatch도 `superpilot`이 맡긴다. 읽기 전용 fallback 전에 안전한 code/test-edit slice를 우선 검토하고, 독립 stage 작업은 병렬화하며, 실패/noop/미회수 시도는 인정하지 않는다.
- 통합된 사용자-facing 동작은 E2E를 proving check 후보로 평가하고, E2E는 기본적으로 로컬에서 실행하며, persistence를 건드리면 격리된 SQLite test DB를 사용한다.
- 명시적 구현 요청에서는 must-have 요구사항이 확인 또는 증거로 확정되고 올바른 spec을 쓸 수 있을 만큼 명확해지면, 원래 사용자 요청을 실행 승인으로 간주한다.
```
